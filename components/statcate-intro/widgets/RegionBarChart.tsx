...
import { geoDimOf, queryRows, yearOrLatest, formatPeriodAxis, isMonthPeriod } from "@/lib/statcate-intro/query";
...
export default function RegionBarChart({ widget, dash, chartHeight }: Props) {
  ...
  const height = chartHeight ?? Math.min(420, Math.max(260, rows.length * 22));
  ...
  return (
    <div className="sector-intro-panel">
      <h4>
        ...
        {title}
        {barYear !== year ? (
          <span> · {isMonthPeriod(barYear) ? formatPeriodAxis(barYear) : barYear}</span>
        ) : null}
      </h4>
      <div className="sector-intro-chart" style={{ height }}>
        <ReactECharts
          option={regionBarOption(rows, (config.palette ?? INTRO_COLORS)[0], {
            lng,
            year: isMonthPeriod(barYear) ? formatPeriodAxis(barYear) : barYear,
