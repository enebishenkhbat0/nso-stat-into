...
/** Monthly PX labels like "2026-07" / "2026-7". */
export function isMonthPeriod(label: string) {
  return /^\d{4}-\d{1,2}$/.test(String(label).trim());
}

/** Axis / picker display: "2026-07" → "2026.07". */
export function formatPeriodAxis(label: string) {
  const match = String(label).trim().match(/^(\d{4})-(\d{1,2})$/);
...
/** Full time labels with data, newest first (months keep "YYYY-MM"). */
export function listPeriods(rows: PxRow[], timeDim: string) {
  const labels = timeLabels(rows, timeDim, true);
  return labels.sort((a, b) => periodSortKey(b) - periodSortKey(a));
}

export function listPeriodOptions(
  packs: { rows: PxRow[]; time?: string }[],
  config: IntroDashboardConfig,
) {
  const periods = [
    ...new Set(packs.flatMap((table) => listPeriods(table.rows, timeDimOf(config, table)))),
  ];
  const monthly = periods.some(isMonthPeriod);
  if (monthly) {
    return {
  ...
  return { monthly: false as const, periods: years };
}
