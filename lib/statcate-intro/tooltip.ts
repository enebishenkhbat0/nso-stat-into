...
import { formatPeriodAxis, isMonthPeriod } from "@/lib/statcate-intro/query";
...
export function introAxisTooltipFormatter(opts?: {
  ...
  return (params: unknown) => {
    ...
    const titleDisplay = isMonthPeriod(title) ? formatPeriodAxis(title) : title;
    const rows = items
    ...
    // Trend charts already use the period as the axis title — don't repeat it.
    const year = opts?.year && opts.year !== title ? opts.year : undefined;

    return introTooltipHtml({ title: titleDisplay, year, yearLabel, rows, showPin: false });
  };
}
  ...
  return (params: unknown) => {
    ...
    if (!Number.isFinite(num)) {
      return introTooltipHtml({
        title: isMonthPeriod(title) ? formatPeriodAxis(title) : title,
        year: opts.year && isMonthPeriod(opts.year) ? formatPeriodAxis(opts.year) : opts.year,
        yearLabel,
        showPin: true,
      });
    }
